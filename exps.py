...
def transform_knobs2vector(knobs_detail, knobs):
    ...
    ys = []
    for key in keys:
        if knobs_detail[key]['type'] in ['integer', 'float']:
...
def transform_vector2knobs(knobs_detail, vector):
    ...
    if len(vector) < len(keys):
        print(f"Warning: vector size {len(vector)} < knobs size {len(keys)}")
        keys = keys[:len(vector)]  # Use only available dimensions
    
    for i in range(len(keys)):
        if knobs_detail[keys[i]]['type'] in ['integer', 'float']:
            minv, maxv = knobs_detail[keys[i]]['min'], knobs_detail[keys[i]]['max']
            tmpv = (maxv - minv) * float(vector[i]) + minv
            # Fix: Convert based on actual type
            if knobs_detail[keys[i]]['type'] == 'integer':
                knobs[keys[i]] = int(tmpv)
            else:  # float
                knobs[keys[i]] = float(tmpv)
...
class LLMTuner(Tuner):
        ...
        for knobs in knobs_set:
            ...
            for key in tmp_keys:
                if key not in keys:
                    ...
                    return False
            for key in tmp_keys:
                if self.knobs_detail[key]['type'] in ['integer', 'float']:
                    v = knobs[key]
    ...
    def _get_next_point_llm_assist(self, candidate_nums=5):
        ...
        UCB = UpperConfidenceBound(gp, beta=0.1, maximize=False if self.objective == 'lat' else True)
        EI = ExpectedImprovement(gp, best_f=target, maximize=False if self.objective == 'lat' else True)
        actual_knob_nums = train_X.shape[1]
        bounds = torch.stack([torch.zeros(actual_knob_nums), torch.ones(actual_knob_nums)])
        with gpytorch.settings.cholesky_jitter(1e-1):
        ...
        if llm_initial_samples is None:
            llm_initial_samples = []  # Handle failed LLM generation
        samples = []
        ...
        if samples:
            ...
            samples = samples.reshape(candidate_nums, 1, actual_knob_nums)
            with gpytorch.settings.cholesky_jitter(1e-1):
