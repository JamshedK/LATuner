...
run_with_timing() {
    local run_id=$1
    local logfile="../logs/tpcc_logs/${DB_NAME}_run${run_id}.log"
    ...
    echo "Running tpcc benchmark on database: $DB_NAME" >> $logfile
    # run main.py 
    cd .. 
...
echo "Starting 5 tpcc runs for: $DB_NAME"
cd scripts
# Run 5 times
...
echo "All runs completed! Check logs in ../logs/tpcc_logs/"